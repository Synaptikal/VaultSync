...
/// Inventory Detail Dialog (Refactored to use ProductProvider)
///
/// Now uses ProductProvider for product lookup instead of direct ApiService calls.
/// Enables offline product information display.
...
class _InventoryDetailDialogState extends State<InventoryDetailDialog> {
  ...
  Future<void> _loadProduct() async {
    try {
      // Use ProductProvider for offline-first product lookup
      final provider = context.read<ProductProvider>();

      // Try to find product in cached list first
      _product = provider.products
          .where((p) => p.productUuid == widget.item.productUuid)
          .firstOrNull;

      // If not in cache, try loading from provider
      if (_product == null) {
              ...
              child: Padding(
                ...
                child: Column(
                  ...
                  children: [
                        ...
                        if (_loading)
                          ...
                          Expanded(
                            child: Column(
                              crossAxisAlignment: CrossAxisAlignment.start,
                              children: [
                                Text(_product?.name ?? 'Unknown Product',
                                    style: Theme.of(context)
                                        .textTheme
                                        .headlineSmall),
                                Text(_product?.setCode ?? '',
                                    style: Theme.of(context)
                                        .textTheme
                                        .titleSmall
                                        ?.copyWith(
                    ...
                    _buildDetailRow(
                        'Variant', widget.item.variantType?.name ?? 'Normal'),
                    _buildDetailRow(
                        'Quantity', widget.item.quantityOnHand.toString()),
                    _buildDetailRow(
                        'Location',
                        widget.item.locationTag.isEmpty
                            ? 'Unassigned'
                            : widget.item.locationTag),
                    if (widget.item.specificPrice != null)
                      _buildDetailRow(
                          'Price', '\$${widget.item.specificPrice}'),
                    if (widget.item.serializedDetails is Map) ...[
                      ...
                      Text(
                          (widget.item.serializedDetails as Map)
                              .entries
                        ...
                        TextButton.icon(
                            ...
                            onPressed: () {
                              ...
                              ScaffoldMessenger.of(context).showSnackBar(
                                const SnackBar(
                                    content: Text(
                                        'Dispose functionality coming soon')),
                              );
                            }),
                          ...
                          onPressed: () async {
                            ...
                            if (result != null && context.mounted) {
                              ScaffoldMessenger.of(context).showSnackBar(
                                SnackBar(
                                  content: Text(
                                      'Printing ${result['quantity']} label(s)...'),
