...
impl AuthRepository {
    ...
    pub async fn insert_user(&self, user_uuid: Uuid, username: &str, password_hash: &str, role: UserRole) -> Result<()> {
        sqlx::query(
    ...
    pub async fn get_user_by_username(&self, username: &str) -> Result<Option<(Uuid, String, String, UserRole)>> {
        let row = sqlx::query("SELECT user_uuid, username, password_hash, role FROM Users WHERE username = ?")
            .bind(username)
    ...
    pub async fn get_user_by_uuid(&self, uuid: Uuid) -> Result<Option<(Uuid, String, String, UserRole)>> {
        let row = sqlx::query("SELECT user_uuid, username, password_hash, role FROM Users WHERE user_uuid = ?")
            .bind(uuid.to_string())
            .fetch_optional(&self.pool)
            .await
            .map_err(|e| crate::errors::VaultSyncError::DatabaseError(e.to_string()))?;
